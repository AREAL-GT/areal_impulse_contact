...
cont_points1 = np.array([[0.05, 0, 0], [-0.05, 0, 0]])
cont_points2 = np.array([[0.05, 0.05, 0], [-0.05, -0.05, 0]])
m = 1
I =  np.eye(3)
e1 = 0.5
e2 = 1
mp_3d_cont =  contact.ImpulseContact(cont_points1, m, I, e=e1, mu=1.5,
                                     micro_override=True,
                                     debug_ctl=contact_debug_ctl)

# Example 3d system dynamics function
def sysdyn_3d_mp(t, x):
    '''
    System dynamics for 3d bouncing multi-point
...
output_ctl1 = { # Output file control
    ...
    'filename': 'ex_3d_mp_bounce.csv', # Name for output file
    'file': True, # Save output file
    'plots': True # Save output plots
}

tspan = (0, 5)
x0 = np.array([0, 0, -1, 0.1, 0, 0])
timestep = 0.01
state_names = ['x', 'y', 'z', 'dx', 'dy', 'dz']

# Run the simulation
sim_test = simulator.Simulator(tspan, x0, sysdyn_3d_mp, timestep=timestep,
